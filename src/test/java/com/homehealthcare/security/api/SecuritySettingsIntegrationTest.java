...
@Transactional
class SecuritySettingsIntegrationTest {
    ...
    @Test
    void ownerCanViewConsolidatedSecuritySettingsContract() throws Exception {
        ...
        mockMvc.perform(get("/api/security/settings")
                        .with(authentication(authenticationFor(ownerMembership))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.mfaPolicy.agencyId").value(agency.getId().toString()))
                .andExpect(jsonPath("$.passwordPolicy.minimumLength").value(12))
                .andExpect(jsonPath("$.sessionPolicy.editable").value(false))
                .andExpect(jsonPath("$.sessionPolicy.scopeBoundary").exists());
    }

    @Test
    void caregiverCannotViewConsolidatedSecuritySettings() throws Exception {
        Agency agency = agencyRepository.saveAndFlush(
                Agency.create("North Star Home Care", UUID.randomUUID().toString(), "America/Chicago", "ops@northstar.example"));
